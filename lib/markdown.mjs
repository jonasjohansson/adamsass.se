...
// never need naming. The `(?!\/)` matters: a leading slash is a path in this
// repo, but two of them are a host, and //evil.example.com is not our site.
function href(url) {
  if (!/^(https?:\/\/|mailto:|\/(?!\/)|assets\/)/.test(url)) {
    throw new Error(`unsupported URL: ${url}, a link must start with https:// or mailto:`);
  }
  return url;
}
...
export function inline(text) {
  ...
  // The lookarounds require a marker to hug its text, so arithmetic and
  // stray asterisks in prose are left alone: 2 * 3 and 4 * 5 is not emphasis.
